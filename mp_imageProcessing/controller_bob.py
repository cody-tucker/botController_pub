...
def on_message_botTaskList(client, userdata, message):
    """
    Check for which bot the task is for, only store it if it is assigned to you
    """
...
def on_message_coordinationFlags(client, userdata, message):
    """
    Check who's id is in the msg. If it is your name than you sent it and it is irrelevant.
    If it is the other bot name than store it in Queue.
    """
    coordData = message.payload.decode("utf-8")
    if len(coordData) > 7: # we would have to get to 4 digit task numbers until this will break... "bobette###"
...
class task_reader():
    ...
    def check_rotation(self, bot_data, goal):
        ...
        if 'idle' in statusData:
            ...
            if abs(req_rotation) < self.angleTolerance and goal == False:
                ...
                return True
            elif abs(req_rotation) < self.goalOrientationTolerance and goal == True:
                print(f'{botname}: YAY! Bot is within tolerance of target angle!')
                return True
            else:
                #req_rotation = gh_rotationDataQ.get()
    ...
    def execute_task(self, item, bot_data):
        ...
        if self.task == 'grab':
            ...
            bobette_pos_X = bot_data[bobetteID]['location'][0]
